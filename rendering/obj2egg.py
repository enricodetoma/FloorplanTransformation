    ...
    def read(self, filename, verbose=False):
        ...
        for line in file.readlines():
            ...
            if tokens[0] in ("Ni"):
                ...
                mat.put(tokens[0], float(tokens[1]))
                continue
            print('file "{}": line {}: unrecognized:'.format(filename, linenumber), tokens)
        file.close()
        if verbose: print("{} materials".format(len(self.materials)), "loaded from", filename)
        return self
    ...
    def read(self, filename, verbose=False):
        ...
        for line in file.readlines():
            ...
            if tokens[0] == "f":
                ...
                continue
            if tokens[0] == "s":
                print("{}:{}: ignoring:".format(filename, linenumber), tokens)
                continue
            if tokens[0] == "l":
                if verbose: print("l:", tokens[1:])
                self.__newpolyline(tokens[1:])
                continue
            print('{}:{}: unknown:'.format(filename, linenumber), tokens)
        file.close()
        return self
